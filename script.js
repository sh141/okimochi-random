...
_o.style.color = colorList[Math.floor(Math.random() * 100) % colorList.length];
const _ki = document.createElement("span");
_ki.textContent = "き";
_ki.style.fontSize = defaultFontSize;
_ki.style.color = colorList[Math.floor(Math.random() * 100) % colorList.length];
const _mo = document.createElement("span");
_mo.textContent = "も";
_mo.style.fontSize = defaultFontSize;
_mo.style.color = colorList[Math.floor(Math.random() * 100) % colorList.length];
...
//自動更新/5s のおきもち
const okimochiReload = () => location.reload(false);
addEventListener("load", function() {
  setTimeout(okimochiReload, 5000);
});
